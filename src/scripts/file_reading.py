...
def compare_messages_by_categorical_fields(msgs1, msgs2, field_names=None, verbose=True):
    ...
    if cat_field_names1 != cat_field_names2:
        result_array.append(
            f"ERROR: Categorical columns in log files (and even their order) must be the same, but they are different:\n"
    ...
    result_array.append(str(cat_clusters1))
    result_array.append(str(cat_clusters2))
    ...
    for stat1 in cat_clusters1:
        ...
        if not cluster_found:
            result_array.append("The following clusters found:")
            cluster_found = True
        noun_count_str = "s" if cluster_count1 > 1 else ""
        verb_count_str = "" if cluster_count1 > 1 else "s"
        if match_found:
            result_array.append(
                f"   The following {len(stat1['cluster_combinations'])} message instance{noun_count_str} " +
                f"occur{verb_count_str} in {100*sum(stat1['cluster_vals'])/stat1['total_count']:00.1f}% of " +
                f"\"{msgs1.filename}\" messages and in {100*sum(stat2['cluster_vals'])/stat2['total_count']:00.1f}% of " +
                f"\"{msgs2.filename}\" messages:"
            )
            for i1, comb in enumerate(stat1['cluster_combinations']):
                result_array.append(f"        [{100*stat1['cluster_vals'][i1]/stat1['total_count']:00.1f}%] in \"{msgs1.filename}\":")
                i2 = stat2['cluster_combinations'].index(comb)
                result_array.append(f"        [{100*stat2['cluster_vals'][i2]/stat2['total_count']:00.1f}%] in \"{msgs2.filename}\":")
                for j, value in enumerate(comb):
                    result_array.append(f"            {field_names[j]}:\t\t{value}")
        else:
            result_array.append(
                f"    \"{msgs1.filename}\" has the following {len(stat1['cluster_combinations'])} message instance{noun_count_str}, " +
            ...
            for i, comb in enumerate(stat1['cluster_combinations']):
                result_array.append(f"        [{100*stat1['cluster_vals'][i]/stat1['total_count']:00.1f}%] in \"{msgs1.filename}\":")
                for j, value in enumerate(comb):
                    result_array.append(f"            {field_names[j]}:\t\t{value}")
    for i2, file2_match_flag in enumerate(file2_match_flags):
        if not file2_match_flag:
            ...
            result_array.append(
                f"    \"{msgs2.filename}\" has the following {len(stat2['cluster_combinations'])} message instance{noun_count_str}, " +
            ...
            for i, comb in enumerate(stat2['cluster_combinations']):
                result_array.append(f"        [{100*stat2['cluster_vals'][i]/stat2['total_count']:00.1f}%] in \"{msgs2.filename}\":")
                for j, value in enumerate(comb):
                    result_array.append(f"            {field_names[j]}:\t\t{value}")
...
class MessageList:
    ...
    def print_categorical_clusters(self, found_cluster_stat, field_names):
        ...
        for stat in found_cluster_stat:
            ...
            to_be_count_str = "are" if len(stat['noise_combinations']) > 1 else "is"
            if len(stat['noise_combinations']) > MAX_NOISE_COMBS:
                announce_ending = f". Fisrt {MAX_NOISE_COMBS} of them:"
            elif len(stat['noise_combinations']) == 1:
                announce_ending = ":"
            else:
                announce_ending = ". All of them:"
            print(f"    There {to_be_count_str} {len(stat['noise_combinations'])} message instance{noun_count_str}, " +
                  f"which occur in the rest of cases{announce_ending}")
